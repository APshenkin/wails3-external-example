...
import { newRuntimeCaller, newRuntimeCallerPostMessage, objectNames } from "./runtime.js";
...
const callPostMessage = newRuntimeCallerPostMessage(objectNames.Call);
...
export function CallPostMessage(options) {
    const id = generateID();
    const result = CancellablePromise.withResolvers();
    callResponses.set(id, { resolve: result.resolve, reject: result.reject });
    const request = callPostMessage(CallBinding, Object.assign({ "call-id": id }, options));
    let running = false;
    request.then(() => {
        running = true;
    }, (err) => {
...
export function ByIDPostMessage(methodID, ...args) {
    return CallPostMessage({ methodID, args });
}
